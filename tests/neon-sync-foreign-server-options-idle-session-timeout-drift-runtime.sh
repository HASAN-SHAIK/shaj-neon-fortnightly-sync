...
create extension postgres_fdw; create table products(id bigint primary key,sku text not null,quantity int not null); create table remote_products(id bigint primary key,sku text not null,quantity int not null); insert into remote_products values(1,'REMOTE-SKU-1',7); create view remote_policy as select current_setting('idle_session_timeout')::text as idle_timeout; grant usage on schema public to cycle_app; grant select on remote_products,remote_policy to cycle_app;
select format('create server retail_loopback foreign data wrapper postgres_fdw options(host ''127.0.0.1'',port ''5432'',dbname %L,options %L)',:'db','-c idle_session_timeout=' || :'mode') \gexec
...
[[ "$sb" == '-c idle_session_timeout=0' && "$db" == '-c idle_session_timeout=5s' && "$sr" == '1|REMOTE-SKU-1|7' && "$dr" == '1|REMOTE-SKU-1|7' && "$se" == '0' && "$de" == '5s' ]] || { echo 'Fixture invalid'; exit 2; }
...
if [[ "$da" == '-c idle_session_timeout=0' && "$row" == '2|SOURCE-SKU-2|11' && "$de2" == '0' ]]; then echo 'NEON_FOREIGN_SERVER_OPTIONS_IDLE_SESSION_TIMEOUT_DRIFT_DETECTED=true'; exit 0; fi
if [[ "$da" == '-c idle_session_timeout=5s' && "$row" == '2|SOURCE-SKU-2|11' && "$de2" == '5s' ]]; then echo 'NEON_FOREIGN_SERVER_OPTIONS_IDLE_SESSION_TIMEOUT_DRIFT_DETECTED=false'; echo 'NEON_FOREIGN_SERVER_OPTIONS_IDLE_SESSION_TIMEOUT_POLICY_DIVERGENCE=true'; exit 1; fi
exit 2