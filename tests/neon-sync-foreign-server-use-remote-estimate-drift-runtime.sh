...
setup_db() {
  local admin_url="$1" remote_estimate="$2"
  psql "$admin_url" -v ON_ERROR_STOP=1 -v remote_estimate="$remote_estimate" <<'SQL'
...
select format('create server retail_loopback foreign data wrapper postgres_fdw options (host ''127.0.0.1'', port ''5432'', dbname %L, use_remote_estimate %L)', current_database(), :'remote_estimate') \gexec
...
server_option() {
  psql "$1" -v ON_ERROR_STOP=1 -Atc "select option_value from pg_options_to_table((select srvoptions from pg_foreign_server where srvname='retail_loopback')) where option_name='use_remote_estimate';"
}
application_probe() {
...
source_option_before="$(server_option "$SOURCE_ADMIN_URL")"
destination_option_before="$(server_option "$DESTINATION_ADMIN_URL")"
...
printf 'BEFORE\nsource use_remote_estimate=%s\ndestination use_remote_estimate=%s\n' "$source_option_before" "$destination_option_before"
...
if [[ "$source_option_before" != false || "$destination_option_before" != true || "$source_app_before" != "$destination_app_before" || "$source_plan_before" == "$destination_plan_before" ]]; then
  echo 'Fixture did not establish isolated use_remote_estimate planning drift with equal application results.' >&2
  exit 2
fi
...
if [[ "$sync_exit" -ne 0 ]]; then
  printf 'NEON_FOREIGN_SERVER_USE_REMOTE_ESTIMATE_DRIFT_SYNC_EXIT=%s\n' "$sync_exit"
  if grep -Eqi 'use_remote_estimate|remote estimate|foreign server.*(estimate|option|drift|mismatch)' <<<"$runtime_output"; then
    echo 'NEON_FOREIGN_SERVER_USE_REMOTE_ESTIMATE_DRIFT_FAIL_CLOSED=true'
    exit 0
  fi
  echo 'NEON_FOREIGN_SERVER_USE_REMOTE_ESTIMATE_DRIFT_FAIL_CLOSED=false'
  exit 1
fi

destination_option_after="$(server_option "$DESTINATION_ADMIN_URL")"
...
printf 'AFTER\ndestination use_remote_estimate=%s\nappended source row=%s\n' "$destination_option_after" "$destination_row"
...
if [[ "$destination_option_after" == false && "$destination_row" == '20001|SOURCE-SKU-20001|11' && "$source_app_after" == "$destination_app_after" && "$source_plan_after" == "$destination_plan_after" ]]; then
  echo 'NEON_FOREIGN_SERVER_USE_REMOTE_ESTIMATE_DRIFT_DETECTED=true'
  exit 0
fi

if [[ "$destination_option_after" == true && "$destination_row" == '20001|SOURCE-SKU-20001|11' && "$source_app_after" == "$destination_app_after" && "$source_plan_before" == "$source_plan_after" && "$destination_plan_before" == "$destination_plan_after" && "$source_plan_after" != "$destination_plan_after" ]]; then
  echo 'NEON_FOREIGN_SERVER_USE_REMOTE_ESTIMATE_DRIFT_DETECTED=false'
  echo 'NEON_FOREIGN_SERVER_USE_REMOTE_ESTIMATE_PLAN_DIVERGENCE=true'
  echo 'Destination retained a different postgres_fdw remote-estimation policy; production synchronization succeeded while real Foreign Scan planner costs remained different from source.' >&2
  exit 1
fi

echo 'Post-sync use_remote_estimate scenario produced an unexpected runtime state.' >&2
exit 2
