...
has_insert_privilege_for_other() {
  psql "$1" -v ON_ERROR_STOP=1 -Atc "select has_table_privilege('cycle_other','public.products','INSERT');"
}
...
set +e
source_before_output="$(psql "$SOURCE_OTHER_URL" -v ON_ERROR_STOP=1 -At -c "insert into public.products values (901,'SOURCE-OTHER-FORBIDDEN',1);" 2>&1)"
source_before_exit=$?
set -e

set +e
destination_before_output="$(psql "$DESTINATION_OTHER_URL" -v ON_ERROR_STOP=1 -At -c "insert into public.products values (901,'DEST-OTHER-BEFORE',1);" 2>&1)"
destination_before_exit=$?
set -e
destination_before_row="$(psql "$DESTINATION_POSTGRES_URL" -v ON_ERROR_STOP=1 -At -F '|' -c "select id,sku,quantity from public.products where id=901;")"
...
set +e
destination_after_output="$(psql "$DESTINATION_OTHER_URL" -v ON_ERROR_STOP=1 -At -c "insert into public.products values (902,'DEST-OTHER-AFTER',1);" 2>&1)"
destination_after_exit=$?
set -e
destination_after_row="$(psql "$DESTINATION_POSTGRES_URL" -v ON_ERROR_STOP=1 -At -F '|' -c "select id,sku,quantity from public.products where id=902;")"
...
test "$destination_before_row" = '901|DEST-OTHER-BEFORE|1'
