...
sb=$(opt "$S"); db=$(opt "$D"); sr=$(psql "$SA" -At -F '|' -c 'select id,sku,quantity from products_remote'); dr=$(psql "$DA" -At -F '|' -c 'select id,sku,quantity from products_remote'); se=$(psql "$SA" -Atc 'select bytea_output from policy_remote'); de=$(psql "$DA" -Atc 'select bytea_output from policy_remote'); printf 'BEFORE source=%s destination=%s source_app=%s destination_app=%s source_bytea_output=%s destination_bytea_output=%s\n' "$sb" "$db" "$sr" "$dr" "$se" "$de"
[[ "$sb" == '-c bytea_output=hex' && "$db" == '-c bytea_output=escape' && "$sr" == '1|REMOTE-SKU-1|7' && "$dr" == '1|REMOTE-SKU-1|7' && "$se" == 'hex' && "$de" == 'escape' ]] || { echo 'Fixture invalid'; exit 2; }
...
if [[ $ec -ne 0 ]]; then echo "NEON_FOREIGN_SERVER_OPTIONS_BYTEA_OUTPUT_DRIFT_SYNC_EXIT=$ec"; if grep -Eqi 'bytea_output|foreign server.*(drift|mismatch|option)' <<<"$out"; then echo 'NEON_FOREIGN_SERVER_OPTIONS_BYTEA_OUTPUT_DRIFT_FAIL_CLOSED=true'; exit 0; fi; exit 1; fi
da=$(opt "$D"); row=$(psql "$D" -At -F '|' -c 'select id,sku,quantity from products where id=2'); de2=$(psql "$DA" -Atc 'select bytea_output from policy_remote'); printf 'AFTER destination=%s row=%s destination_bytea_output=%s\nNEON_FOREIGN_SERVER_OPTIONS_BYTEA_OUTPUT_DRIFT_SYNC_EXIT=%s\n' "$da" "$row" "$de2" "$ec"
if [[ "$da" == '-c bytea_output=hex' && "$row" == '2|SOURCE-SKU-2|11' && "$de2" == 'hex' ]]; then echo 'NEON_FOREIGN_SERVER_OPTIONS_BYTEA_OUTPUT_DRIFT_DETECTED=true'; exit 0; fi
if [[ "$da" == '-c bytea_output=escape' && "$row" == '2|SOURCE-SKU-2|11' && "$de2" == 'escape' ]]; then echo 'NEON_FOREIGN_SERVER_OPTIONS_BYTEA_OUTPUT_DRIFT_DETECTED=false'; echo 'NEON_FOREIGN_SERVER_OPTIONS_BYTEA_OUTPUT_POLICY_DIVERGENCE=true'; exit 1; fi
exit 2