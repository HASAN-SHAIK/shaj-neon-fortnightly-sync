...
probe_plan() {
  ...
  plan="$(psql "$url" -X -v ON_ERROR_STOP=1 -At -c 'explain (costs off) select count(*) from public.join_left l join public.join_right r on r.id=l.id where l.id <= 100;' | tr '\n' ';')"
  result="$(psql "$url" -X -v ON_ERROR_STOP=1 -At -c 'select count(*) from public.join_left l join public.join_right r on r.id=l.id where l.id <= 100;')"
...
[[ "$source_probe" == on\|*"Nested Loop"*"|count=100|15000|SOURCE-SKU-15000|0" ]] || { echo "Source did not use expected Nested Loop: $source_probe" >&2; exit 2; }
[[ "$destination_probe" == off\|*"Hash Join"*"|count=100|15000|SOURCE-SKU-15000|0" && "$destination_probe" != *"Nested Loop"* ]] || { echo "Destination did not use expected Hash Join fallback: $destination_probe" >&2; exit 2; }
...
if [[ "$source_probe_after" != on\|*"Nested Loop"*"|count=100|15000|SOURCE-SKU-15000|0" || "$destination_probe_after" != off\|*"Hash Join"*"|count=100|15000|SOURCE-SKU-15000|0" || "$destination_probe_after" == *"Nested Loop"* ]]; then
