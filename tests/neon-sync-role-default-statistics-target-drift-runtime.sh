...
alter role cycle_app set default_statistics_target = 1000;
create database cycle_d_source;
SQL
psql "$DST_ROOT" -v ON_ERROR_STOP=1 <<'SQL'
create role cycle_app login;
alter role cycle_app set default_statistics_target = 1;
...
create table public.stats_probe (id bigint primary key, sample_key integer not null, payload text not null);
alter table public.stats_probe owner to cycle_app;
...
insert into public.stats_probe
select g, g, md5(g::text) from generate_series(1,10000) g;
SQL
done
...
analyze_probe() {
  local url="$1"
  psql "$url" -X -v ON_ERROR_STOP=1 -q -c 'analyze public.stats_probe;'
  psql "$url" -X -v ON_ERROR_STOP=1 -At -c "select coalesce(array_length(histogram_bounds,1),0) from pg_stats where schemaname='public' and tablename='stats_probe' and attname='sample_key';"
}

assert_runtime_boundary() {
  local src_setting="$1" dst_setting="$2" src_row="$3" dst_row="$4" src_hist="$5" dst_hist="$6"
  [[ "$src_setting" == '1000' ]] || return 1
  [[ "$dst_setting" == '1' ]] || return 1
  [[ "$src_row" == '15000|SOURCE-SKU-15000|0' ]] || return 1
  [[ "$dst_row" == '15000|SOURCE-SKU-15000|0' ]] || return 1
  [[ "$src_hist" =~ ^[0-9]+$ && "$dst_hist" =~ ^[0-9]+$ ]] || return 1
  (( src_hist > dst_hist )) || return 1
  (( dst_hist >= 2 )) || return 1
}
...
src_hist="$(analyze_probe "$SRC_APP")"; dst_hist="$(analyze_probe "$DST_APP")"
printf 'BEFORE\nsource catalog setting=%s\ndestination catalog setting=%s\nsource effective setting=%s\ndestination effective setting=%s\nsource ordinary row=%s\ndestination ordinary row=%s\nsource histogram bounds=%s\ndestination histogram bounds=%s\n' "$src_catalog" "$dst_catalog" "$src_effective" "$dst_effective" "$src_row" "$dst_row" "$src_hist" "$dst_hist"
[[ "${src_catalog,,}" == ${SETTING}=* && "${dst_catalog,,}" == ${SETTING}=* ]] || { echo 'Catalog fixture did not persist both role settings.' >&2; exit 2; }
assert_runtime_boundary "$src_effective" "$dst_effective" "$src_row" "$dst_row" "$src_hist" "$dst_hist" || { echo 'Fixture did not establish the required statistics-target application boundary.' >&2; exit 2; }
...
printf '%s\n' "$out"
if [[ "$rc" -ne 0 ]]; then
  printf 'NEON_ROLE_DEFAULT_STATISTICS_TARGET_DRIFT_SYNC_EXIT=%s\n' "$rc"
  if grep -Eqi 'default_statistics_target|pg_db_role_setting|role setting' <<<"$out"; then
    echo 'NEON_ROLE_DEFAULT_STATISTICS_TARGET_DRIFT_FAIL_CLOSED=true'; exit 0
  fi
  echo 'NEON_ROLE_DEFAULT_STATISTICS_TARGET_DRIFT_FAIL_CLOSED=false'; exit 1
fi
...
src_hist_after="$(analyze_probe "$SRC_APP")"; dst_hist_after="$(analyze_probe "$DST_APP")"
dst_appended="$(psql "$DST_ADMIN" -v ON_ERROR_STOP=1 -At -F '|' -c 'select id,sku,quantity from public.products where id=20001;')"
printf 'AFTER\ndestination catalog setting=%s\nsource effective setting=%s\ndestination effective setting=%s\nappended source row=%s\nsource histogram bounds=%s\ndestination histogram bounds=%s\nNEON_ROLE_DEFAULT_STATISTICS_TARGET_DRIFT_SYNC_EXIT=%s\n' "$dst_catalog_after" "$src_effective_after" "$dst_effective_after" "$dst_appended" "$src_hist_after" "$dst_hist_after" "$rc"
[[ "$dst_appended" == '20001|SOURCE-SKU-20001|11' ]] || { echo 'Legitimate application row did not synchronize.' >&2; exit 2; }
assert_runtime_boundary "$src_effective_after" "$dst_effective_after" "$src_row_after" "$dst_row_after" "$src_hist_after" "$dst_hist_after" || { echo 'Statistics-target application boundary did not persist after sync.' >&2; exit 2; }
echo 'NEON_ROLE_DEFAULT_STATISTICS_TARGET_DRIFT_DETECTED=false'
exit 1
