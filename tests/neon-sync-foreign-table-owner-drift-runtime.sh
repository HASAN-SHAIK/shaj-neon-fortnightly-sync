...
setup_db() {
  local admin_url="$1" dbname="$2" owner="$3"
  psql "$admin_url" -v ON_ERROR_STOP=1 -v dbname="$dbname" -v owner="$owner" <<'SQL'
...
select format('alter foreign table public.products_remote owner to %I', :'owner') \gexec
grant select on public.products_remote to cycle_app;
SQL
}

setup_db "$SOURCE_ADMIN_URL" cycle_d_source cycle_owner
...
foreign_table_owner() {
  psql "$1" -v ON_ERROR_STOP=1 -Atc "select pg_get_userbyid(c.relowner) from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relname='products_remote' and c.relkind='f';"
}
app_probe() {
  psql "$1" -v ON_ERROR_STOP=1 -At -F '|' -c "select id,sku,quantity from public.products_remote order by id;"
}
break_foreign_table_as_other() {
  psql "$1" -v ON_ERROR_STOP=1 -c "alter foreign table public.products_remote options (set table_name 'missing_products');"
}
restore_destination_foreign_table() {
  psql "$DESTINATION_ADMIN_URL" -v ON_ERROR_STOP=1 -c "alter foreign table public.products_remote options (set table_name 'products'); alter foreign table public.products_remote owner to cycle_other;"
}

source_owner_before="$(foreign_table_owner "$SOURCE_ADMIN_URL")"
destination_owner_before="$(foreign_table_owner "$DESTINATION_ADMIN_URL")"
source_app_before="$(app_probe "$SOURCE_APP_URL")"
destination_app_before="$(app_probe "$DESTINATION_APP_URL")"
set +e
source_alter_output="$(break_foreign_table_as_other "$SOURCE_OTHER_URL" 2>&1)"; source_alter_exit=$?
destination_alter_output="$(break_foreign_table_as_other "$DESTINATION_OTHER_URL" 2>&1)"; destination_alter_exit=$?
destination_app_after_output="$(app_probe "$DESTINATION_APP_URL" 2>&1)"; destination_app_after_exit=$?
set -e
printf 'BEFORE\nsource foreign table owner=%s\ndestination foreign table owner=%s\n' "$source_owner_before" "$destination_owner_before"
...
if [[ "$source_owner_before" != cycle_owner || "$destination_owner_before" != cycle_other || "$source_app_before" != '1|BASE-SKU-1|7' || "$destination_app_before" != '1|BASE-SKU-1|7' || "$source_alter_exit" -eq 0 || "$destination_alter_exit" -ne 0 || "$destination_app_after_exit" -eq 0 ]]; then
  echo 'Fixture did not establish isolated foreign-table ownership drift.' >&2
  exit 2
fi

restore_destination_foreign_table
[[ "$(foreign_table_owner "$DESTINATION_ADMIN_URL")" == cycle_other ]] || exit 2
...
if [[ "$sync_exit" -ne 0 ]]; then
  printf 'NEON_FOREIGN_TABLE_OWNER_DRIFT_SYNC_EXIT=%s\n' "$sync_exit"
  if grep -Eqi 'foreign table.*(owner|ownership).*(incompatib|drift|mismatch)|(owner|ownership).*foreign table' <<<"$runtime_output"; then
    echo 'NEON_FOREIGN_TABLE_OWNER_DRIFT_FAIL_CLOSED=true'
    exit 0
  fi
  echo 'NEON_FOREIGN_TABLE_OWNER_DRIFT_FAIL_CLOSED=false'
  exit 1
fi

destination_owner_after="$(foreign_table_owner "$DESTINATION_ADMIN_URL")"
...
set +e
source_alter_after_output="$(break_foreign_table_as_other "$SOURCE_OTHER_URL" 2>&1)"; source_alter_after_exit=$?
destination_alter_after_output="$(break_foreign_table_as_other "$DESTINATION_OTHER_URL" 2>&1)"; destination_alter_after_exit=$?
...
if [[ "$destination_owner_after" == cycle_owner && "$destination_row_2" == '2|SOURCE-SKU-2|11' && "$source_alter_after_exit" -ne 0 && "$destination_alter_after_exit" -ne 0 ]]; then
  echo 'NEON_FOREIGN_TABLE_OWNER_DRIFT_DETECTED=true'
  exit 0
fi
...
if [[ "$destination_owner_after" == cycle_other && "$destination_row_2" == '2|SOURCE-SKU-2|11' && "$destination_app_before_final" == $'1|BASE-SKU-1|7\n2|SOURCE-SKU-2|11' && "$source_alter_after_exit" -ne 0 && "$destination_alter_after_exit" -eq 0 && "$destination_app_after_final_exit" -ne 0 ]]; then
  echo 'NEON_FOREIGN_TABLE_OWNER_DRIFT_DETECTED=false'
  echo 'Destination retained foreign-table owner authority denied on source; owner-only option mutation broke the real application foreign-table path.' >&2
  exit 1
fi

echo 'Post-sync foreign-table ownership scenario produced an unexpected runtime state.' >&2
exit 2
