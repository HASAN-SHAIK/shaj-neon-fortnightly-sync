...
setup_db() {
  local admin_url="$1" dbname="$2" first_async="$3"
  psql "$admin_url" -v ON_ERROR_STOP=1 -v dbname="$dbname" -v first_async="$first_async" <<'SQL'
...
select format('create server retail_async_a foreign data wrapper postgres_fdw options (host ''127.0.0.1'', port ''5432'', dbname %L, async_capable %L)', :'dbname', :'first_async') \gexec
...
create foreign table public.async_products_b partition of public.async_products for values from (100) to (200)
  server retail_async_b options (schema_name 'public', table_name 'remote_b');
...
source_option_before="$(server_async "$SOURCE_ADMIN_URL")"
destination_option_before="$(server_async "$DESTINATION_ADMIN_URL")"
...
printf 'BEFORE\nsource foreign server async_capable=%s\ndestination foreign server async_capable=%s\n' "$source_option_before" "$destination_option_before"
...
if [[ "$source_option_before" != true || "$destination_option_before" != false || "$source_read_before" != '2|18' || "$destination_read_before" != '2|18' || "$source_async_before" -ne 2 || "$destination_async_before" -ne 1 ]]; then
  echo 'Fixture did not establish isolated foreign-server async_capable drift with observable asynchronous Append planning.' >&2
  exit 2
fi
...
if [[ "$sync_exit" -ne 0 ]]; then
  printf 'NEON_FOREIGN_SERVER_ASYNC_CAPABLE_DRIFT_SYNC_EXIT=%s\n' "$sync_exit"
  if grep -Eqi 'foreign server.*(async_capable|async capable|option|incompatib|drift|mismatch)|(async_capable|async capable|option|drift|mismatch).*foreign server' <<<"$runtime_output"; then
    echo 'NEON_FOREIGN_SERVER_ASYNC_CAPABLE_DRIFT_FAIL_CLOSED=true'
    exit 0
  fi
  echo 'NEON_FOREIGN_SERVER_ASYNC_CAPABLE_DRIFT_FAIL_CLOSED=false'
  exit 1
fi

destination_option_after="$(server_async "$DESTINATION_ADMIN_URL")"
...
if [[ "$destination_option_after" == true && "$destination_row_2" == '2|SOURCE-SKU-2|11' && "$source_read_after" == '2|18' && "$destination_read_after" == '2|18' && "$source_async_after" -eq 2 && "$destination_async_after" -eq 2 ]]; then
  echo 'NEON_FOREIGN_SERVER_ASYNC_CAPABLE_DRIFT_DETECTED=true'
  exit 0
fi

if [[ "$destination_option_after" == false && "$destination_row_2" == '2|SOURCE-SKU-2|11' && "$source_read_after" == '2|18' && "$destination_read_after" == '2|18' && "$source_async_after" -eq 2 && "$destination_async_after" -eq 1 ]]; then
  echo 'NEON_FOREIGN_SERVER_ASYNC_CAPABLE_DRIFT_DETECTED=false'
  echo 'NEON_FOREIGN_SERVER_ASYNC_CAPABLE_PLAN_DIVERGENCE=true'
  echo 'Destination retained server-level async_capable=false; production synchronization succeeded while identical application results used a different synchronous/asynchronous Append execution policy.' >&2
  exit 1
fi

echo 'Post-sync foreign-server async_capable scenario produced an unexpected runtime state.' >&2
exit 2
