...
create index gin_fuzzy_probe_search_idx on public.gin_fuzzy_probe using gin (search_vector);
...
insert into public.gin_fuzzy_probe(id, body)
select g, 'apple common token ' || g::text from generate_series(1,20000) g;
...
where search_vector @@ plainto_tsquery('simple','apple');
SQL
}
...
assert_runtime_boundary() {
  ...
  (( dst_count > 0 && dst_count < src_count && dst_count < 1000 )) || return 1
...
assert_runtime_boundary "$src_effective" "$dst_effective" "$src_row" "$dst_row" "$src_probe" "$dst_probe" "$src_plan" "$dst_plan" || { echo 'Fixture did not establish required GIN fuzzy-search result boundary.' >&2; exit 2; }
...
assert_runtime_boundary "$src_effective_after" "$dst_effective_after" "$src_row_after" "$dst_row_after" "$src_probe_after" "$dst_probe_after" "$src_plan_after" "$dst_plan_after" || { echo 'GIN fuzzy-search result boundary did not persist after sync.' >&2; exit 2; }
