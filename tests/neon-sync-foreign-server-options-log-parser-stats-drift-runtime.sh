...
sb=$(opt "$S"); db=$(opt "$D"); sr=$(psql "$SA" -At -F '|' -c 'select id,sku,quantity from products_remote'); dr=$(psql "$DA" -At -F '|' -c 'select id,sku,quantity from products_remote'); sp=$(psql "$SA" -Atc 'select log_parser_stats from policy_remote'); dp=$(psql "$DA" -Atc 'select log_parser_stats from policy_remote'); printf 'BEFORE source=%s destination=%s source_app=%s destination_app=%s source_policy=%s destination_policy=%s\n' "$sb" "$db" "$sr" "$dr" "$sp" "$dp"
[[ "$sb" == '-c log_parser_stats=off' && "$db" == '-c log_parser_stats=on' && "$sr" == '1|REMOTE-SKU-1|7' && "$dr" == '1|REMOTE-SKU-1|7' && "$sp" == 'off' && "$dp" == 'on' ]] || { echo 'Fixture invalid'; exit 2; }
...
if [[ $ec -ne 0 ]]; then echo "NEON_FOREIGN_SERVER_OPTIONS_LOG_PARSER_STATS_DRIFT_SYNC_EXIT=$ec"; if grep -Eqi 'log_parser_stats|foreign server.*(drift|mismatch|option)' <<<"$out"; then echo 'NEON_FOREIGN_SERVER_OPTIONS_LOG_PARSER_STATS_DRIFT_FAIL_CLOSED=true'; exit 0; fi; exit 1; fi
da=$(opt "$D"); row=$(psql "$D" -At -F '|' -c 'select id,sku,quantity from products where id=2'); dp2=$(psql "$DA" -Atc 'select log_parser_stats from policy_remote'); printf 'AFTER destination=%s row=%s destination_policy=%s\nNEON_FOREIGN_SERVER_OPTIONS_LOG_PARSER_STATS_DRIFT_SYNC_EXIT=%s\n' "$da" "$row" "$dp2" "$ec"
if [[ "$da" == '-c log_parser_stats=off' && "$row" == '2|SOURCE-SKU-2|11' && "$dp2" == 'off' ]]; then echo 'NEON_FOREIGN_SERVER_OPTIONS_LOG_PARSER_STATS_DRIFT_DETECTED=true'; exit 0; fi
if [[ "$da" == '-c log_parser_stats=on' && "$row" == '2|SOURCE-SKU-2|11' && "$dp2" == 'on' ]]; then echo 'NEON_FOREIGN_SERVER_OPTIONS_LOG_PARSER_STATS_DRIFT_DETECTED=false'; echo 'NEON_FOREIGN_SERVER_OPTIONS_LOG_PARSER_STATS_POLICY_DIVERGENCE=true'; exit 1; fi
exit 2
