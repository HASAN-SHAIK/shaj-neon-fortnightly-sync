...
setup_destination() {
  {
    create_operator_class_sql
    cat <<'SQL'
alter operator class public.retail_int4_ops using btree owner to cycle_other;
create table public.products(id bigint primary key, sku text not null, quantity integer not null);
insert into public.products
select g, 'BASE-SKU-' || g::text, g % 100 from generate_series(1,2000) g;
create index products_quantity_retail_idx on public.products using btree(quantity public.retail_int4_ops);
...
drop_as_other() {
  psql "$1" -v ON_ERROR_STOP=1 -c "drop operator class public.retail_int4_ops using btree cascade;"
}
restore_destination_operator_class() {
  {
    create_operator_class_sql
    cat <<'SQL'
alter operator class public.retail_int4_ops using btree owner to cycle_other;
...
source_owner_before="$(operator_class_owner "$SOURCE_ADMIN_URL")"
destination_owner_before="$(operator_class_owner "$DESTINATION_ADMIN_URL")"
...
printf 'destination index after owner drop=%s\ndestination app count after owner drop=%s\ndestination plan after owner drop exit=%s\ndestination plan after owner drop=%s\n' "$destination_index_after_drop" "$destination_count_after_drop" "$destination_plan_after_drop_exit" "$destination_plan_after_drop"

if [[ "$source_owner_before" != cycle_owner || "$destination_owner_before" != cycle_other || "$source_index_before" != true || "$destination_index_before" != true || "$source_count_before" != 20 || "$destination_count_before" != 20 || "$source_drop_exit" -eq 0 || "$destination_drop_exit" -ne 0 || "$destination_index_after_drop" != false || "$destination_count_after_drop" != 20 ]]; then
  echo 'Fixture did not establish isolated operator-class ownership drift.' >&2
  exit 2
fi
if ! grep -Eq 'Index Scan|Index Only Scan|Bitmap Index Scan' <<<"$source_plan_before" || ! grep -Eq 'Index Scan|Index Only Scan|Bitmap Index Scan' <<<"$destination_plan_before"; then
  echo 'Baseline application plan did not use the custom operator-class index.' >&2
  exit 2
fi
if grep -Eq 'products_quantity_retail_idx' <<<"$destination_plan_after_drop"; then
  echo 'Destination owner mutation did not remove the dependent application index.' >&2
  exit 2
fi

restore_destination_operator_class
if [[ "$(operator_class_owner "$DESTINATION_ADMIN_URL")" != cycle_other || "$(index_present "$DESTINATION_ADMIN_URL")" != true ]]; then
...
if [[ "$sync_exit" -ne 0 ]]; then
  printf 'NEON_OPERATOR_CLASS_OWNER_DRIFT_SYNC_EXIT=%s\n' "$sync_exit"
  if grep -Eqi 'operator class.*(owner|ownership).*(incompatib|drift|mismatch)|(owner|ownership).*operator class' <<<"$runtime_output"; then
    echo 'NEON_OPERATOR_CLASS_OWNER_DRIFT_FAIL_CLOSED=true'
    exit 0
  fi
  echo 'NEON_OPERATOR_CLASS_OWNER_DRIFT_FAIL_CLOSED=false'
  exit 1
fi

destination_owner_after="$(operator_class_owner "$DESTINATION_ADMIN_URL")"
...
printf 'AFTER\ndestination operator class owner=%s\nappended source row=%s\n' "$destination_owner_after" "$destination_row_2001"
...
printf 'destination index after final owner drop=%s\ndestination app count after final owner drop=%s\ndestination plan after final owner drop=%s\nNEON_OPERATOR_CLASS_OWNER_DRIFT_SYNC_EXIT=%s\n' "$destination_index_after_final_drop" "$destination_count_after_final_drop" "$destination_plan_after_final_drop" "$sync_exit"

if [[ "$destination_owner_after" == cycle_owner && "$destination_row_2001" == '2001|SOURCE-SKU-2001|11' && "$source_drop_after_exit" -ne 0 && "$destination_drop_after_exit" -ne 0 && "$destination_index_before_final_drop" == true ]]; then
  echo 'NEON_OPERATOR_CLASS_OWNER_DRIFT_DETECTED=true'
  exit 0
fi

if [[ "$destination_owner_after" == cycle_other && "$destination_row_2001" == '2001|SOURCE-SKU-2001|11' && "$source_drop_after_exit" -ne 0 && "$destination_drop_after_exit" -eq 0 && "$destination_index_before_final_drop" == true && "$destination_index_after_final_drop" == false && "$destination_count_before_final_drop" == 20 && "$destination_count_after_final_drop" == 20 ]] && grep -Eq 'products_quantity_retail_idx' <<<"$destination_plan_before_final_drop" && ! grep -Eq 'products_quantity_retail_idx' <<<"$destination_plan_after_final_drop"; then
  echo 'NEON_OPERATOR_CLASS_OWNER_DRIFT_DETECTED=false'
  echo 'Destination retained operator-class owner authority that source assigns to a different role; owner-only CASCADE drop removed the application index and changed the real query plan.' >&2
  exit 1
fi

echo 'Post-sync operator-class ownership scenario produced an unexpected runtime state.' >&2
exit 2
