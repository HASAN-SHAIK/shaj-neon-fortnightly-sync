...
select format('create server retail_loopback foreign data wrapper postgres_fdw options (host ''127.0.0.1'', port ''5432'', dbname %L, truncatable %L)', :'dbname', :'truncatable') \gexec
...
server_truncatable_option() {
  psql "$1" -v ON_ERROR_STOP=1 -Atc "select option_value from pg_options_to_table((select srvoptions from pg_foreign_server where srvname='retail_loopback')) where option_name='truncatable';"
}
truncate_probe() {
  local url="$1"
...
source_option_before="$(server_truncatable_option "$SOURCE_ADMIN_URL")"
destination_option_before="$(server_truncatable_option "$DESTINATION_ADMIN_URL")"
...
printf 'BEFORE\nsource foreign server truncatable=%s\ndestination foreign server truncatable=%s\n' "$source_option_before" "$destination_option_before"
...
if [[ "$source_option_before" != true || "$destination_option_before" != false || "$source_read_before" != '1|BASE-SKU-1|7' || "$destination_read_before" != '1|BASE-SKU-1|7' || "$source_truncate_before" != $'0\n1' || "$destination_truncate_before_exit" -eq 0 ]]; then
  echo 'Fixture did not establish isolated foreign-server truncatable drift.' >&2
  exit 2
fi
...
if [[ "$sync_exit" -ne 0 ]]; then
  printf 'NEON_FOREIGN_SERVER_TRUNCATABLE_DRIFT_SYNC_EXIT=%s\n' "$sync_exit"
  if grep -Eqi 'foreign server.*(truncatable|truncate|option|incompatib|drift|mismatch)|(truncatable|truncate|option|drift|mismatch).*foreign server' <<<"$runtime_output"; then
    echo 'NEON_FOREIGN_SERVER_TRUNCATABLE_DRIFT_FAIL_CLOSED=true'
    exit 0
  fi
  echo 'NEON_FOREIGN_SERVER_TRUNCATABLE_DRIFT_FAIL_CLOSED=false'
  exit 1
fi

destination_option_after="$(server_truncatable_option "$DESTINATION_ADMIN_URL")"
...
if [[ "$destination_option_after" == true && "$destination_row_2" == '2|SOURCE-SKU-2|11' && "$source_truncate_after" == $'0\n2' && "$destination_truncate_after_exit" -eq 0 ]]; then
  echo 'NEON_FOREIGN_SERVER_TRUNCATABLE_DRIFT_DETECTED=true'
  exit 0
fi
...
if [[ "$destination_option_after" == false && "$destination_row_2" == '2|SOURCE-SKU-2|11' && "$source_truncate_after" == $'0\n2' && "$destination_truncate_after_exit" -ne 0 ]]; then
  echo 'NEON_FOREIGN_SERVER_TRUNCATABLE_DRIFT_DETECTED=false'
  echo 'NEON_FOREIGN_SERVER_TRUNCATABLE_PATH_BROKEN=true'
  echo 'Destination retained server-level truncatable=false; production synchronization succeeded while the real application foreign-table TRUNCATE path remained unavailable.' >&2
  exit 1
fi

echo 'Post-sync foreign-server truncatable scenario produced an unexpected runtime state.' >&2
exit 2
