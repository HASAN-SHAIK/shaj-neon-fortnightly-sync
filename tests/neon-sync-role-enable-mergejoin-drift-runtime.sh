...
alter role cycle_app set enable_mergejoin = on;
...
role_setting() {
  psql "$1" -v ON_ERROR_STOP=1 -At -c "select cfg from pg_db_role_setting s join pg_roles r on r.oid=s.setrole cross join lateral unnest(s.setconfig) cfg where r.rolname='cycle_app' and s.setdatabase=0 and lower(cfg) like 'enable_mergejoin=%';"
}
probe_plan() {
  local url="$1" mergejoin plan result row
  mergejoin="$(psql "$url" -X -v ON_ERROR_STOP=1 -At -c 'show enable_mergejoin;')"
  ...
  printf '%s|%s|count=%s|%s' "$mergejoin" "$plan" "$result" "$row"
}
...
[[ "${source_setting,,}" == 'enable_mergejoin=on' && "${destination_setting,,}" == 'enable_mergejoin=off' ]] || { echo 'Fixture did not establish enable_mergejoin drift.' >&2; exit 2; }
[[ "$source_probe" == on\|*"Merge Join"*"|count=20000|15000|SOURCE-SKU-15000|0" ]] || { echo "Source did not use expected Merge Join: $source_probe" >&2; exit 2; }
[[ "$destination_probe" == off\|*"Nested Loop"*"|count=20000|15000|SOURCE-SKU-15000|0" && "$destination_probe" != *"Merge Join"* ]] || { echo "Destination did not use expected Nested Loop fallback: $destination_probe" >&2; exit 2; }
...
if [[ "$sync_exit" -ne 0 ]]; then
  printf 'NEON_ROLE_ENABLE_MERGEJOIN_DRIFT_SYNC_EXIT=%s\n' "$sync_exit"
  if grep -Eqi 'enable_mergejoin|pg_db_role_setting|role setting' <<<"$runtime_output"; then echo 'NEON_ROLE_ENABLE_MERGEJOIN_DRIFT_FAIL_CLOSED=true'; exit 0; fi
  echo 'NEON_ROLE_ENABLE_MERGEJOIN_DRIFT_FAIL_CLOSED=false'; exit 1
fi
...
printf 'AFTER\ndestination role setting=%s\nappended source row=%s\nsource app join probe=%s\ndestination app join probe=%s\nNEON_ROLE_ENABLE_MERGEJOIN_DRIFT_SYNC_EXIT=%s\n' "$destination_setting_after" "$destination_row" "$source_probe_after" "$destination_probe_after" "$sync_exit"
if [[ "$source_probe_after" != on\|*"Merge Join"*"|count=20000|15000|SOURCE-SKU-15000|0" || "$destination_probe_after" != off\|*"Nested Loop"*"|count=20000|15000|SOURCE-SKU-15000|0" || "$destination_probe_after" == *"Merge Join"* ]]; then
  echo 'Post-sync fixture/data no longer isolates the intended enable_mergejoin planner boundary.' >&2
  exit 2
fi
if [[ "${destination_setting_after,,}" == 'enable_mergejoin=on' && "$destination_row" == '20001|SOURCE-SKU-20001|11' && "$destination_probe_after" == on\|*"Merge Join"* ]]; then echo 'NEON_ROLE_ENABLE_MERGEJOIN_DRIFT_DETECTED=true'; exit 0; fi
echo 'NEON_ROLE_ENABLE_MERGEJOIN_DRIFT_DETECTED=false'
exit 1
