...
setup_db() {
 local admin_url="$1" dbname="$2" interval="$3"
 psql "$admin_url" -v ON_ERROR_STOP=1 -v dbname="$dbname" -v interval="$interval" <<'SQL'
...
select format('create server retail_loopback foreign data wrapper postgres_fdw options (host ''127.0.0.1'', port ''5432'', dbname %L, keepalives_interval %L)', :'dbname', :'interval') \gexec
...
setup_db "$DESTINATION_ADMIN_URL" cycle_d_destination 30
...
server_interval(){ psql "$1" -v ON_ERROR_STOP=1 -Atc "select option_value from pg_options_to_table((select srvoptions from pg_foreign_server where srvname='retail_loopback')) where option_name='keepalives_interval';"; }
...
src_before="$(server_interval "$SOURCE_ADMIN_URL")"; dst_before="$(server_interval "$DESTINATION_ADMIN_URL")"
sr="$(app_read "$SOURCE_APP_URL")"; dr="$(app_read "$DESTINATION_APP_URL")"; sw="$(app_write_probe "$SOURCE_APP_URL" "$SOURCE_ADMIN_URL" 9301)"; dw="$(app_write_probe "$DESTINATION_APP_URL" "$DESTINATION_ADMIN_URL" 9302)"
printf 'BEFORE\nsource foreign server keepalives_interval=%s\ndestination foreign server keepalives_interval=%s\nsource app read=%s\ndestination app read=%s\nsource write probe=%s\ndestination write probe=%s\n' "$src_before" "$dst_before" "$sr" "$dr" "$sw" "$dw"
[[ "$src_before" == 2 && "$dst_before" == 30 && "$sr" == '1|REMOTE-SKU-1|7' && "$dr" == '1|REMOTE-SKU-1|7' && "$sw" == $'1\n0' && "$dw" == $'1\n0' ]] || { echo 'Fixture invalid' >&2; exit 2; }
...
if [[ "$sync_exit" -ne 0 ]]; then printf 'NEON_FOREIGN_SERVER_KEEPALIVES_INTERVAL_DRIFT_SYNC_EXIT=%s\n' "$sync_exit"; if grep -Eqi 'foreign server.*(keepalives_interval|keepalive|option|incompatib|drift|mismatch)|(keepalives_interval|keepalive|option|drift|mismatch).*foreign server' <<<"$runtime_output"; then echo 'NEON_FOREIGN_SERVER_KEEPALIVES_INTERVAL_DRIFT_FAIL_CLOSED=true'; exit 0; fi; echo 'NEON_FOREIGN_SERVER_KEEPALIVES_INTERVAL_DRIFT_FAIL_CLOSED=false'; exit 1; fi
dst_after="$(server_interval "$DESTINATION_ADMIN_URL")"; row2="$(psql "$DESTINATION_ADMIN_URL" -v ON_ERROR_STOP=1 -At -F '|' -c 'select id,sku,quantity from public.products where id=2;')"; sr2="$(app_read "$SOURCE_APP_URL")"; dr2="$(app_read "$DESTINATION_APP_URL")"; sw2="$(app_write_probe "$SOURCE_APP_URL" "$SOURCE_ADMIN_URL" 9311)"; dw2="$(app_write_probe "$DESTINATION_APP_URL" "$DESTINATION_ADMIN_URL" 9312)"
printf 'AFTER\ndestination foreign server keepalives_interval=%s\nappended source row=%s\nsource app read=%s\ndestination app read=%s\nsource write probe=%s\ndestination write probe=%s\nNEON_FOREIGN_SERVER_KEEPALIVES_INTERVAL_DRIFT_SYNC_EXIT=%s\n' "$dst_after" "$row2" "$sr2" "$dr2" "$sw2" "$dw2" "$sync_exit"
if [[ "$dst_after" == 2 && "$row2" == '2|SOURCE-SKU-2|11' && "$sr2" == '1|REMOTE-SKU-1|7' && "$dr2" == '1|REMOTE-SKU-1|7' && "$sw2" == $'1\n0' && "$dw2" == $'1\n0' ]]; then echo 'NEON_FOREIGN_SERVER_KEEPALIVES_INTERVAL_DRIFT_DETECTED=true'; exit 0; fi
if [[ "$dst_after" == 30 && "$row2" == '2|SOURCE-SKU-2|11' && "$sr2" == '1|REMOTE-SKU-1|7' && "$dr2" == '1|REMOTE-SKU-1|7' && "$sw2" == $'1\n0' && "$dw2" == $'1\n0' ]]; then echo 'NEON_FOREIGN_SERVER_KEEPALIVES_INTERVAL_DRIFT_DETECTED=false'; echo 'NEON_FOREIGN_SERVER_KEEPALIVES_INTERVAL_POLICY_DIVERGENCE=true'; exit 1; fi
