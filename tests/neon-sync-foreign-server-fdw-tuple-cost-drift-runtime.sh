...
setup_db() {
  local admin_url="$1" tuple_cost="$2"
  psql "$admin_url" -v ON_ERROR_STOP=1 -v tuple_cost="$tuple_cost" <<'SQL'
create extension postgres_fdw;
create table public.products(id bigint primary key, sku text not null, quantity integer not null);
grant usage on schema public to cycle_app;
grant select on public.products to cycle_app;
select format('create server retail_loopback foreign data wrapper postgres_fdw options (host ''127.0.0.1'', port ''5432'', dbname %L, fdw_tuple_cost %L)', current_database(), :'tuple_cost') \gexec
...
setup_db "$SOURCE_ADMIN_URL" 0.01
...
server_tuple_cost() {
  psql "$1" -v ON_ERROR_STOP=1 -Atc "select option_value from pg_options_to_table((select srvoptions from pg_foreign_server where srvname='retail_loopback')) where option_name='fdw_tuple_cost';"
}
...
source_option_before="$(server_tuple_cost "$SOURCE_ADMIN_URL")"
destination_option_before="$(server_tuple_cost "$DESTINATION_ADMIN_URL")"
...
printf 'BEFORE\nsource fdw_tuple_cost=%s\ndestination fdw_tuple_cost=%s\n' "$source_option_before" "$destination_option_before"
...
if [[ "$source_option_before" != 0.01 || "$destination_option_before" != 10 || "$source_app_before" != "$destination_app_before" || "$source_plan_before" == "$destination_plan_before" ]]; then
  echo 'Fixture did not establish isolated fdw_tuple_cost planning drift with equal application results.' >&2
  exit 2
fi
...
if [[ "$sync_exit" -ne 0 ]]; then
  printf 'NEON_FOREIGN_SERVER_FDW_TUPLE_COST_DRIFT_SYNC_EXIT=%s\n' "$sync_exit"
  if grep -Eqi 'fdw_tuple_cost|tuple cost|foreign server.*(cost|option|drift|mismatch)' <<<"$runtime_output"; then
    echo 'NEON_FOREIGN_SERVER_FDW_TUPLE_COST_DRIFT_FAIL_CLOSED=true'
    exit 0
  fi
  echo 'NEON_FOREIGN_SERVER_FDW_TUPLE_COST_DRIFT_FAIL_CLOSED=false'
  exit 1
fi

destination_option_after="$(server_tuple_cost "$DESTINATION_ADMIN_URL")"
...
if [[ "$destination_option_after" == 0.01 && "$destination_row_2001" == '2001|SOURCE-SKU-2001|11' && "$source_app_after" == "$destination_app_after" && "$source_plan_after" == "$destination_plan_after" ]]; then
  echo 'NEON_FOREIGN_SERVER_FDW_TUPLE_COST_DRIFT_DETECTED=true'
  exit 0
fi

if [[ "$destination_option_after" == 10 && "$destination_row_2001" == '2001|SOURCE-SKU-2001|11' && "$source_app_after" == "$destination_app_after" && "$source_plan_before" == "$source_plan_after" && "$destination_plan_before" == "$destination_plan_after" && "$source_plan_after" != "$destination_plan_after" ]]; then
  echo 'NEON_FOREIGN_SERVER_FDW_TUPLE_COST_DRIFT_DETECTED=false'
  echo 'NEON_FOREIGN_SERVER_FDW_TUPLE_COST_PLAN_DIVERGENCE=true'
  echo 'Destination retained a different postgres_fdw tuple-cost policy; production synchronization succeeded while real Foreign Scan planner costs remained different from source.' >&2
  exit 1
fi

echo 'Post-sync fdw_tuple_cost scenario produced an unexpected runtime state.' >&2
exit 2
