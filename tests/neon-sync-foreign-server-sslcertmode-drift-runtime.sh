...
setup(){ local u="$1" db="$2" certmode="$3"; psql "$u" -v ON_ERROR_STOP=1 -v db="$db" -v certmode="$certmode" <<'SQL'
...
select format('create server retail_loopback foreign data wrapper postgres_fdw options(host ''127.0.0.1'',port ''5432'',dbname %L,sslmode ''require'',sslcertmode %L)',:'db',:'certmode') \gexec
...
setup "$S" cycle_d_source allow; setup "$D" cycle_d_destination require
psql "$S" -c "insert into products values(1,'BASE-SKU-1',7)"; psql "$D" -c "insert into products values(1,'BASE-SKU-1',7)"
opt(){ psql "$1" -Atc "select option_value from pg_options_to_table((select srvoptions from pg_foreign_server where srvname='retail_loopback')) where option_name='sslcertmode';"; }
...
[[ "$sb" == allow && "$db" == require && "$sr" == '1|REMOTE-SKU-1|7' && "$dr" == '1|REMOTE-SKU-1|7' ]] || { echo 'Fixture invalid'; exit 2; }
...
if [[ $ec -ne 0 ]]; then echo "NEON_FOREIGN_SERVER_SSLCERTMODE_DRIFT_SYNC_EXIT=$ec"; if grep -Eqi 'sslcertmode|foreign server.*(drift|mismatch|option)' <<<"$out"; then echo 'NEON_FOREIGN_SERVER_SSLCERTMODE_DRIFT_FAIL_CLOSED=true'; exit 0; fi; exit 1; fi
da=$(opt "$D"); row=$(psql "$D" -At -F '|' -c 'select id,sku,quantity from products where id=2'); dr2=$(psql "$DA" -At -F '|' -c 'select id,sku,quantity from products_remote'); printf 'AFTER destination=%s row=%s destination_app=%s\nNEON_FOREIGN_SERVER_SSLCERTMODE_DRIFT_SYNC_EXIT=%s\n' "$da" "$row" "$dr2" "$ec"
if [[ "$da" == allow && "$row" == '2|SOURCE-SKU-2|11' && "$dr2" == '1|REMOTE-SKU-1|7' ]]; then echo 'NEON_FOREIGN_SERVER_SSLCERTMODE_DRIFT_DETECTED=true'; exit 0; fi
if [[ "$da" == require && "$row" == '2|SOURCE-SKU-2|11' && "$dr2" == '1|REMOTE-SKU-1|7' ]]; then echo 'NEON_FOREIGN_SERVER_SSLCERTMODE_DRIFT_DETECTED=false'; echo 'NEON_FOREIGN_SERVER_SSLCERTMODE_POLICY_DIVERGENCE=true'; exit 1; fi
exit 2