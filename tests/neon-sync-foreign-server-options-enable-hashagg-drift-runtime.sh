...
if [[ $ec -ne 0 ]]; then echo "NEON_FOREIGN_SERVER_OPTIONS_ENABLE_HASHAGG_DRIFT_SYNC_EXIT=$ec"; if grep -Eqi 'enable_hashagg|foreign server.*(drift|mismatch|option)' <<<"$out"; then echo 'NEON_FOREIGN_SERVER_OPTIONS_ENABLE_HASHAGG_DRIFT_FAIL_CLOSED=true'; exit 0; fi; exit 1; fi
da=$(opt "$D"); row=$(psql "$D" -At -F '|' -c 'select id,sku,quantity from products where id=2'); dp2=$(psql "$DA" -Atc 'select enable_hashagg from policy_remote'); printf 'AFTER destination=%s row=%s destination_policy=%s\nNEON_FOREIGN_SERVER_OPTIONS_ENABLE_HASHAGG_DRIFT_SYNC_EXIT=%s\n' "$da" "$row" "$dp2" "$ec"
if [[ "$da" == '-c enable_hashagg=on' && "$row" == '2|SOURCE-SKU-2|11' && "$dp2" == 'on' ]]; then echo 'NEON_FOREIGN_SERVER_OPTIONS_ENABLE_HASHAGG_DRIFT_DETECTED=true'; exit 0; fi
if [[ "$da" == '-c enable_hashagg=off' && "$row" == '2|SOURCE-SKU-2|11' && "$dp2" == 'off' ]]; then echo 'NEON_FOREIGN_SERVER_OPTIONS_ENABLE_HASHAGG_DRIFT_DETECTED=false'; echo 'NEON_FOREIGN_SERVER_OPTIONS_ENABLE_HASHAGG_POLICY_DIVERGENCE=true'; exit 1; fi
exit 2
