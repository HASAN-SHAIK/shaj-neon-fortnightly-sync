...
setup_db() {
 local admin_url="$1" dbname="$2" mode="$3"
 psql "$admin_url" -v ON_ERROR_STOP=1 -v dbname="$dbname" -v mode="$mode" <<'SQL'
...
select format('create server retail_loopback foreign data wrapper postgres_fdw options (host ''127.0.0.1'', port ''5432'', dbname %L, gssencmode %L)', :'dbname', :'mode') \gexec
...
server_mode(){ psql "$1" -v ON_ERROR_STOP=1 -Atc "select option_value from pg_options_to_table((select srvoptions from pg_foreign_server where srvname='retail_loopback')) where option_name='gssencmode';"; }
src_before="$(server_mode "$SOURCE_ADMIN_URL")"; dst_before="$(server_mode "$DESTINATION_ADMIN_URL")"
...
if [[ "$sync_exit" -ne 0 ]]; then printf 'NEON_FOREIGN_SERVER_GSSENCMODE_DRIFT_SYNC_EXIT=%s\n' "$sync_exit"; if grep -Eqi 'foreign server.*(gssencmode|GSS|option|incompatib|drift|mismatch)|(gssencmode|GSS|option|drift|mismatch).*foreign server' <<<"$runtime_output"; then echo 'NEON_FOREIGN_SERVER_GSSENCMODE_DRIFT_FAIL_CLOSED=true'; exit 0; fi; echo 'NEON_FOREIGN_SERVER_GSSENCMODE_DRIFT_FAIL_CLOSED=false'; exit 1; fi
dst_after="$(server_mode "$DESTINATION_ADMIN_URL")"; row2="$(psql "$DESTINATION_ADMIN_URL" -v ON_ERROR_STOP=1 -At -F '|' -c 'select id,sku,quantity from public.products where id=2;')"
...
printf 'AFTER\ndestination foreign server gssencmode=%s\nappended source row=%s\ndestination app exit=%s\ndestination app=%s\nNEON_FOREIGN_SERVER_GSSENCMODE_DRIFT_SYNC_EXIT=%s\n' "$dst_after" "$row2" "$dst_app_exit2" "$dr2" "$sync_exit"
if [[ "$dst_after" == disable && "$row2" == '2|SOURCE-SKU-2|11' && "$dst_app_exit2" -eq 0 ]]; then echo 'NEON_FOREIGN_SERVER_GSSENCMODE_DRIFT_DETECTED=true'; exit 0; fi
if [[ "$dst_after" == require && "$row2" == '2|SOURCE-SKU-2|11' && "$dst_app_exit2" -ne 0 ]]; then echo 'NEON_FOREIGN_SERVER_GSSENCMODE_DRIFT_DETECTED=false'; echo 'NEON_FOREIGN_SERVER_GSSENCMODE_APPLICATION_PATH_BROKEN=true'; exit 1; fi
