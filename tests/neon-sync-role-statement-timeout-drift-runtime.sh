...
probe_query() {
  ...
  set +e
  local output
  output="$(psql "$url" -v ON_ERROR_STOP=1 -At -F '|' -c "select id, sku, quantity from public.products where id=1; select pg_sleep(0.25);" 2>&1)"
  local code=$?
  set -e
  printf '%s|%s\n' "$code" "$output"
}
...
source_probe="$(probe_query "$SOURCE_APP_URL")"
destination_probe="$(probe_query "$DESTINATION_APP_URL")"

printf 'BEFORE\nsource role setting=%s\ndestination role setting=%s\n' "$source_setting" "$destination_setting"
printf 'source app query=%s\ndestination app query=%s\n' "$source_probe" "$destination_probe"

if [[ "$source_setting" != 'statement_timeout=100ms' || "$destination_setting" != 'statement_timeout=0' ]]; then
  echo 'Fixture did not establish role statement_timeout drift.' >&2
  exit 2
fi
if [[ "$source_probe" != 1\|* || "$source_probe" != *'canceling statement due to statement timeout'* || "$source_probe" != *'1|SOURCE-SKU-1|7'* ]]; then
  echo 'Source application query did not demonstrate statement timeout as expected.' >&2
  exit 2
fi
if [[ "$destination_probe" != 0\|* || "$destination_probe" != *'1|SOURCE-SKU-1|7'* ]]; then
  echo 'Destination application query did not demonstrate unlimited statement runtime.' >&2
  exit 2
fi
...
if [[ "$sync_exit" -ne 0 ]]; then
  ...
  if grep -Eqi 'statement_timeout|pg_db_role_setting|role setting' <<<"$runtime_output"; then
    echo 'NEON_ROLE_STATEMENT_TIMEOUT_DRIFT_FAIL_CLOSED=true'
    exit 0
  fi
  echo 'NEON_ROLE_STATEMENT_TIMEOUT_DRIFT_FAIL_CLOSED=false'
  exit 1
fi
...
destination_probe_after="$(probe_query "$DESTINATION_APP_URL")"
...
if [[ "$destination_setting_after" == 'statement_timeout=100ms' && "$destination_row_2" == '2|SOURCE-SKU-2|11' && "$destination_probe_after" == 1\|* && "$destination_probe_after" == *'canceling statement due to statement timeout'* ]]; then
  echo 'NEON_ROLE_STATEMENT_TIMEOUT_DRIFT_DETECTED=true'
  exit 0
fi
