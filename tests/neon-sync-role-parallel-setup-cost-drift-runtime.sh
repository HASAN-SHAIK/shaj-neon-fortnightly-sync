...
create role cycle_app login;
alter role cycle_app set parallel_setup_cost = 1000000000;
...
create table public.parallel_setup_cost_probe (
  id bigint primary key,
  bucket integer not null,
  payload text not null
);
grant usage on schema public to cycle_app;
grant select on public.products, public.parallel_setup_cost_probe to cycle_app;
insert into public.products
select g, 'SOURCE-SKU-' || lpad(g::text,5,'0'), g % 100 from generate_series(1,20000) g;
insert into public.parallel_setup_cost_probe
select g, g % 100, repeat(md5(g::text), 8)
from generate_series(1,600000) g;
analyze public.products;
analyze public.parallel_setup_cost_probe;
SQL
done
psql "$SRC_ADMIN" -v ON_ERROR_STOP=1 -c "insert into public.products values (20001,'SOURCE-SKU-20001',11); analyze public.products;"

role_setting() {
  psql "$1" -v ON_ERROR_STOP=1 -At -c "select cfg from pg_db_role_setting s join pg_roles r on r.oid=s.setrole cross join lateral unnest(s.setconfig) cfg where r.rolname='cycle_app' and s.setdatabase=0 and lower(cfg) like 'parallel_setup_cost=%';"
}
...
select id,bucket,payload from public.parallel_setup_cost_probe where bucket < 50;
select md5(string_agg(id::text || ':' || bucket::text || ':' || payload, ',' order by id))
from public.parallel_setup_cost_probe where bucket < 50;
...
assert_source() {
  ...
  grep -Eq 'Gather|Gather Merge' <<<"$v" || return 1
  grep -Eq 'Parallel Seq Scan on parallel_setup_cost_probe' <<<"$v" || return 1
  assert_common "$v"
}
assert_destination() {
  local v="$1"
...
printf 'BEFORE\nsource role setting=%s\ndestination role setting=%s\nsource app parallel-setup-cost probe=%s\ndestination app parallel-setup-cost probe=%s\n' "$src_setting" "$dst_setting" "$src_probe" "$dst_probe"
[[ "${src_setting,,}" == 'parallel_setup_cost=0' && "${dst_setting,,}" == 'parallel_setup_cost=1000000000' ]] || { echo 'Fixture did not establish parallel_setup_cost drift.' >&2; exit 2; }
assert_source "$src_probe" || { echo "Source did not use parallel execution under parallel_setup_cost=0: $src_probe" >&2; exit 2; }
assert_destination "$dst_probe" || { echo "Destination did not use serial execution under high parallel_setup_cost: $dst_probe" >&2; exit 2; }
...
printf '%s\n' "$out"
if [[ "$rc" -ne 0 ]]; then
  printf 'NEON_ROLE_PARALLEL_SETUP_COST_DRIFT_SYNC_EXIT=%s\n' "$rc"
  if grep -Eqi 'parallel_setup_cost|pg_db_role_setting|role setting' <<<"$out"; then
    echo 'NEON_ROLE_PARALLEL_SETUP_COST_DRIFT_FAIL_CLOSED=true'; exit 0
  fi
  echo 'NEON_ROLE_PARALLEL_SETUP_COST_DRIFT_FAIL_CLOSED=false'; exit 1
fi
...
printf 'AFTER\ndestination role setting=%s\nappended source row=%s\nsource app parallel-setup-cost probe=%s\ndestination app parallel-setup-cost probe=%s\nNEON_ROLE_PARALLEL_SETUP_COST_DRIFT_SYNC_EXIT=%s\n' "$dst_setting_after" "$dst_row" "$src_after" "$dst_after" "$rc"
[[ "${dst_setting_after,,}" == 'parallel_setup_cost=1000000000' ]] || { echo 'Destination setting changed unexpectedly.' >&2; exit 2; }
[[ "$dst_row" == '20001|SOURCE-SKU-20001|11' ]] || { echo 'Legitimate application row did not synchronize.' >&2; exit 2; }
assert_source "$src_after" || { echo 'Source parallel_setup_cost behavior did not persist.' >&2; exit 2; }
assert_destination "$dst_after" || { echo 'Destination parallel_setup_cost behavior did not persist.' >&2; exit 2; }
[[ "$(digest "$src_after")" == "$(digest "$dst_after")" ]] || { echo 'Probe results differ after sync.' >&2; exit 2; }
echo 'NEON_ROLE_PARALLEL_SETUP_COST_DRIFT_DETECTED=false'
exit 1
