...
create role cycle_app login;
alter role cycle_app set idle_in_transaction_session_timeout = '100ms';
...
idle_tx_probe() {
  local url="$1" tmp_sql tmp_out rc out
  tmp_sql="$(mktemp)"
  tmp_out="$(mktemp)"
  cat >"$tmp_sql" <<'PSQL'
...
PSQL
  set +e
  psql "$url" -X -f "$tmp_sql" >"$tmp_out" 2>&1
  rc=$?
  set -e
  out="$(tr '\n' ';' <"$tmp_out")"
...
assert_runtime_boundary() {
  ...
  grep -Fq 'after-idle' <<<"$src_probe" || return 1
  grep -Eq '^[1-9][0-9]*\|' <<<"$dst_probe" || return 1
  grep -Eqi 'idle-in-transaction timeout|idle in transaction.*timeout|terminating connection' <<<"$dst_probe" || return 1
}
...
printf '%s\n' "$out"
if [[ "$rc" -ne 0 ]]; then
  printf 'NEON_ROLE_IDLE_IN_TRANSACTION_TIMEOUT_DRIFT_SYNC_EXIT=%s\n' "$rc"
  if grep -Eqi 'idle_in_transaction_session_timeout|pg_db_role_setting|role setting' <<<"$out"; then
    echo 'NEON_ROLE_IDLE_IN_TRANSACTION_TIMEOUT_DRIFT_FAIL_CLOSED=true'; exit 0
  fi
  echo 'NEON_ROLE_IDLE_IN_TRANSACTION_TIMEOUT_DRIFT_FAIL_CLOSED=false'; exit 1
fi
...
src_probe_after="$(idle_tx_probe "$SRC_APP")"; dst_probe_after="$(idle_tx_probe "$DST_APP")"
printf 'AFTER\ndestination catalog setting=%s\nsource effective setting=%s\ndestination effective setting=%s\nappended source row=%s\nsource idle transaction probe=%s\ndestination idle transaction probe=%s\nNEON_ROLE_IDLE_IN_TRANSACTION_TIMEOUT_DRIFT_SYNC_EXIT=%s\n' "$dst_catalog_after" "$src_effective_after" "$dst_effective_after" "$dst_appended" "$src_probe_after" "$dst_probe_after" "$rc"
[[ "$dst_appended" == '20001|SOURCE-SKU-20001|11' ]] || { echo 'Legitimate application row did not synchronize.' >&2; exit 2; }
assert_runtime_boundary "$src_effective_after" "$dst_effective_after" "$src_row_after" "$dst_row_after" "$src_probe_after" "$dst_probe_after" || { echo 'idle-in-transaction application boundary did not persist after sync.' >&2; exit 2; }
echo 'NEON_ROLE_IDLE_IN_TRANSACTION_TIMEOUT_DRIFT_DETECTED=false'
exit 1
