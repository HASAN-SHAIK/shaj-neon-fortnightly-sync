...
create table public.date_parse_probe (id bigint primary key, business_date date not null, note text not null);
alter table public.date_parse_probe owner to cycle_app;
...
effective_setting() { psql "$1" -X -v ON_ERROR_STOP=1 -At -c "show DateStyle;"; }
ordinary_row() { psql "$1" -X -v ON_ERROR_STOP=1 -At -F '|' -c 'select id,sku,quantity from public.products where id=15000;'; }
reset_probe() { psql "$1" -X -v ON_ERROR_STOP=1 -c 'truncate table public.date_parse_probe;' >/dev/null; }
application_date_write() {
  psql "$1" -X -v ON_ERROR_STOP=1 -At -F '|' <<'SQL'
insert into public.date_parse_probe(id,business_date,note) values (1,'01/02/2026','ambiguous-literal');
select id,business_date,note from public.date_parse_probe where id=1;
SQL
}

assert_runtime_boundary() {
  local src_setting="$1" dst_setting="$2" src_row="$3" dst_row="$4" src_probe="$5" dst_probe="$6"
  [[ "${src_setting^^}" == *'MDY'* ]] || return 1
  [[ "${dst_setting^^}" == *'DMY'* ]] || return 1
  [[ "$src_row" == '15000|SOURCE-SKU-15000|0' ]] || return 1
  [[ "$dst_row" == '15000|SOURCE-SKU-15000|0' ]] || return 1
  [[ "$src_probe" == *'1|2026-01-02|ambiguous-literal'* ]] || return 1
  [[ "$dst_probe" == *'1|2026-02-01|ambiguous-literal'* ]] || return 1
}
...
reset_probe "$SRC_APP"; reset_probe "$DST_APP"
src_probe="$(application_date_write "$SRC_APP")"; dst_probe="$(application_date_write "$DST_APP")"
printf 'BEFORE\nsource catalog setting=%s\ndestination catalog setting=%s\nsource effective setting=%s\ndestination effective setting=%s\nsource ordinary row=%s\ndestination ordinary row=%s\nsource application date write=%s\ndestination application date write=%s\n' "$src_catalog" "$dst_catalog" "$src_effective" "$dst_effective" "$src_row" "$dst_row" "$(tr '\n' ';' <<<"$src_probe")" "$(tr '\n' ';' <<<"$dst_probe")"
[[ "${src_catalog,,}" == ${SETTING}=* && "${dst_catalog,,}" == ${SETTING}=* ]] || { echo 'Catalog fixture did not persist both role DateStyle settings.' >&2; exit 2; }
assert_runtime_boundary "$src_effective" "$dst_effective" "$src_row" "$dst_row" "$src_probe" "$dst_probe" || { echo 'Fixture did not establish required application date-parsing boundary.' >&2; exit 2; }

set +e
out="$(EXCLUDED_TABLES='public.date_parse_probe' SOURCE_DATABASE_URL="$SRC_ADMIN" DESTINATION_DATABASE_URL="$DST_ADMIN" bash scripts/neon-sync/append-sync.sh 2>&1)"; rc=$?
set -e
printf '%s\n' "$out"
if [[ "$rc" -ne 0 ]]; then
  printf 'NEON_ROLE_DATESTYLE_DRIFT_SYNC_EXIT=%s\n' "$rc"
  if grep -Eqi 'datestyle|pg_db_role_setting|role setting' <<<"$out"; then
...
reset_probe "$SRC_APP"; reset_probe "$DST_APP"
src_probe_after="$(application_date_write "$SRC_APP")"; dst_probe_after="$(application_date_write "$DST_APP")"
...
assert_runtime_boundary "$src_effective_after" "$dst_effective_after" "$src_row_after" "$dst_row_after" "$src_probe_after" "$dst_probe_after" || { echo 'Application date-parsing boundary did not persist after sync.' >&2; exit 2; }
