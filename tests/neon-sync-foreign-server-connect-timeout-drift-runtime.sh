...
select format('create server retail_loopback foreign data wrapper postgres_fdw options (host ''127.0.0.1'', port ''5432'', dbname %L, connect_timeout %L)', :'dbname', :'timeout') \gexec
...
setup_db "$SOURCE_ADMIN_URL" cycle_d_source 1
...
server_timeout() {
  psql "$1" -v ON_ERROR_STOP=1 -Atc "select option_value from pg_options_to_table((select srvoptions from pg_foreign_server where srvname='retail_loopback')) where option_name='connect_timeout';"
}
...
source_write_before="$(app_write_probe "$SOURCE_APP_URL" "$SOURCE_ADMIN_URL" 9001)"
destination_write_before="$(app_write_probe "$DESTINATION_APP_URL" "$DESTINATION_ADMIN_URL" 9002)"

printf 'BEFORE\nsource foreign server connect_timeout=%s\ndestination foreign server connect_timeout=%s\n' "$source_timeout_before" "$destination_timeout_before"
printf 'source app read=%s\ndestination app read=%s\n' "$source_read_before" "$destination_read_before"
printf 'source write probe=%s\ndestination write probe=%s\n' "$source_write_before" "$destination_write_before"

if [[ "$source_timeout_before" != 1 || "$destination_timeout_before" != 10 || "$source_read_before" != '1|REMOTE-SKU-1|7' || "$destination_read_before" != '1|REMOTE-SKU-1|7' || "$source_write_before" != $'1\n0' || "$destination_write_before" != $'1\n0' ]]; then
  echo 'Fixture did not establish isolated connect_timeout drift with equivalent real FDW read/write behavior.' >&2
  exit 2
fi
...
if [[ "$sync_exit" -ne 0 ]]; then
  printf 'NEON_FOREIGN_SERVER_CONNECT_TIMEOUT_DRIFT_SYNC_EXIT=%s\n' "$sync_exit"
  if grep -Eqi 'foreign server.*(connect_timeout|connect timeout|option|incompatib|drift|mismatch)|(connect_timeout|connect timeout|option|drift|mismatch).*foreign server' <<<"$runtime_output"; then
    echo 'NEON_FOREIGN_SERVER_CONNECT_TIMEOUT_DRIFT_FAIL_CLOSED=true'
    exit 0
  fi
  echo 'NEON_FOREIGN_SERVER_CONNECT_TIMEOUT_DRIFT_FAIL_CLOSED=false'
  exit 1
fi
...
source_write_after="$(app_write_probe "$SOURCE_APP_URL" "$SOURCE_ADMIN_URL" 9011)"
destination_write_after="$(app_write_probe "$DESTINATION_APP_URL" "$DESTINATION_ADMIN_URL" 9012)"
...
if [[ "$destination_timeout_after" == 1 && "$destination_row_2" == '2|SOURCE-SKU-2|11' && "$source_read_after" == '1|REMOTE-SKU-1|7' && "$destination_read_after" == '1|REMOTE-SKU-1|7' && "$source_write_after" == $'1\n0' && "$destination_write_after" == $'1\n0' ]]; then
  echo 'NEON_FOREIGN_SERVER_CONNECT_TIMEOUT_DRIFT_DETECTED=true'
  exit 0
fi

if [[ "$destination_timeout_after" == 10 && "$destination_row_2" == '2|SOURCE-SKU-2|11' && "$source_read_after" == '1|REMOTE-SKU-1|7' && "$destination_read_after" == '1|REMOTE-SKU-1|7' && "$source_write_after" == $'1\n0' && "$destination_write_after" == $'1\n0' ]]; then
  echo 'NEON_FOREIGN_SERVER_CONNECT_TIMEOUT_DRIFT_DETECTED=false'
  echo 'NEON_FOREIGN_SERVER_CONNECT_TIMEOUT_POLICY_DIVERGENCE=true'
  echo 'Destination retained connect_timeout=10 while source requires connect_timeout=1; production synchronization reported success and both live FDW paths remained functionally usable.' >&2
  exit 1
fi

echo 'Post-sync connect_timeout scenario produced an unexpected runtime state.' >&2
exit 2
