...
sb=$(opt "$S"); db=$(opt "$D"); sr=$(psql "$SA" -At -F '|' -c 'select id,sku,quantity from products_remote'); dr=$(psql "$DA" -At -F '|' -c 'select id,sku,quantity from products_remote'); sp=$(psql "$SA" -Atc 'select idle_in_transaction_session_timeout from policy_remote'); dp=$(psql "$DA" -Atc 'select idle_in_transaction_session_timeout from policy_remote'); printf 'BEFORE source=%s destination=%s source_app=%s destination_app=%s source_timeout=%s destination_timeout=%s\n' "$sb" "$db" "$sr" "$dr" "$sp" "$dp"
[[ "$sb" == '-c idle_in_transaction_session_timeout=200ms' && "$db" == '-c idle_in_transaction_session_timeout=0' && "$sr" == '1|REMOTE-SKU-1|7' && "$dr" == '1|REMOTE-SKU-1|7' && "$sp" == '200ms' && "$dp" == '0' ]] || { echo 'Fixture invalid'; exit 2; }
...
if [[ $ec -ne 0 ]]; then echo "NEON_FOREIGN_SERVER_OPTIONS_IDLE_IN_TRANSACTION_TIMEOUT_DRIFT_SYNC_EXIT=$ec"; if grep -Eqi 'idle_in_transaction_session_timeout|foreign server.*(drift|mismatch|option)' <<<"$out"; then echo 'NEON_FOREIGN_SERVER_OPTIONS_IDLE_IN_TRANSACTION_TIMEOUT_DRIFT_FAIL_CLOSED=true'; exit 0; fi; exit 1; fi
da=$(opt "$D"); row=$(psql "$D" -At -F '|' -c 'select id,sku,quantity from products where id=2'); dp2=$(psql "$DA" -Atc 'select idle_in_transaction_session_timeout from policy_remote'); printf 'AFTER destination=%s row=%s destination_timeout=%s\nNEON_FOREIGN_SERVER_OPTIONS_IDLE_IN_TRANSACTION_TIMEOUT_DRIFT_SYNC_EXIT=%s\n' "$da" "$row" "$dp2" "$ec"
if [[ "$da" == '-c idle_in_transaction_session_timeout=200ms' && "$row" == '2|SOURCE-SKU-2|11' && "$dp2" == '200ms' ]]; then echo 'NEON_FOREIGN_SERVER_OPTIONS_IDLE_IN_TRANSACTION_TIMEOUT_DRIFT_DETECTED=true'; exit 0; fi
if [[ "$da" == '-c idle_in_transaction_session_timeout=0' && "$row" == '2|SOURCE-SKU-2|11' && "$dp2" == '0' ]]; then echo 'NEON_FOREIGN_SERVER_OPTIONS_IDLE_IN_TRANSACTION_TIMEOUT_DRIFT_DETECTED=false'; echo 'NEON_FOREIGN_SERVER_OPTIONS_IDLE_IN_TRANSACTION_TIMEOUT_POLICY_DIVERGENCE=true'; exit 1; fi
exit 2