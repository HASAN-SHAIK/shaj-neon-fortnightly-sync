...
alter role cycle_app set idle_session_timeout = '200ms';
create database cycle_d_source;
SQL
psql "$DESTINATION_ROOT_URL" -v ON_ERROR_STOP=1 <<'SQL'
create role cycle_app login password 'cycle-app-password';
alter role cycle_app set idle_session_timeout = '0';
...
source_probe="$(probe_idle_session "$SOURCE_APP_URL")"
destination_probe="$(probe_idle_session "$DESTINATION_APP_URL")"
printf 'BEFORE\nsource role setting=%s\ndestination role setting=%s\nsource idle session=%s\ndestination idle session=%s\n' "$source_setting" "$destination_setting" "$source_probe" "$destination_probe"

if [[ "$source_setting" != 'idle_session_timeout=200ms' || "$destination_setting" != 'idle_session_timeout=0' ]]; then
  echo 'Fixture did not establish idle session timeout drift.' >&2; exit 2
fi
if [[ "$source_probe" != *'terminating connection due to idle-session timeout'* ]]; then
  echo 'Source session was not terminated by idle session timeout.' >&2; exit 2
fi
...
if [[ "$sync_exit" -ne 0 ]]; then
  printf 'NEON_ROLE_IDLE_SESSION_TIMEOUT_DRIFT_SYNC_EXIT=%s\n' "$sync_exit"
  if grep -Eqi 'idle_session_timeout|pg_db_role_setting|role setting' <<<"$runtime_output"; then
    echo 'NEON_ROLE_IDLE_SESSION_TIMEOUT_DRIFT_FAIL_CLOSED=true'; exit 0
  fi
  echo 'NEON_ROLE_IDLE_SESSION_TIMEOUT_DRIFT_FAIL_CLOSED=false'; exit 1
fi
...
source_probe_after="$(probe_idle_session "$SOURCE_APP_URL")"
destination_probe_after="$(probe_idle_session "$DESTINATION_APP_URL")"
printf 'AFTER\ndestination role setting=%s\nappended source row=%s\nsource idle session=%s\ndestination idle session=%s\nNEON_ROLE_IDLE_SESSION_TIMEOUT_DRIFT_SYNC_EXIT=%s\n' "$destination_setting_after" "$destination_row_2" "$source_probe_after" "$destination_probe_after" "$sync_exit"

if [[ "$destination_setting_after" == 'idle_session_timeout=200ms' && "$destination_row_2" == '2|SOURCE-SKU-2|11' && "$destination_probe_after" == *'terminating connection due to idle-session timeout'* ]]; then
  echo 'NEON_ROLE_IDLE_SESSION_TIMEOUT_DRIFT_DETECTED=true'; exit 0
fi
echo 'NEON_ROLE_IDLE_SESSION_TIMEOUT_DRIFT_DETECTED=false'
exit 1
