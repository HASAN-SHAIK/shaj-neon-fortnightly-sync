...
sb=$(opt "$S"); db=$(opt "$D"); sr=$(psql "$SA" -At -F '|' -c 'select id,sku,quantity from products_remote'); dr=$(psql "$DA" -At -F '|' -c 'select id,sku,quantity from products_remote'); sp=$(psql "$SA" -Atc 'select temp_buffers from policy_remote'); dp=$(psql "$DA" -Atc 'select temp_buffers from policy_remote'); printf 'BEFORE source=%s destination=%s source_app=%s destination_app=%s source_temp_buffers=%s destination_temp_buffers=%s\n' "$sb" "$db" "$sr" "$dr" "$sp" "$dp"
[[ "$sb" == '-c temp_buffers=16MB' && "$db" == '-c temp_buffers=8MB' && "$sr" == '1|REMOTE-SKU-1|7' && "$dr" == '1|REMOTE-SKU-1|7' && "$sp" == '16MB' && "$dp" == '8MB' ]] || { echo 'Fixture invalid'; exit 2; }
...
if [[ $ec -ne 0 ]]; then echo "NEON_FOREIGN_SERVER_OPTIONS_TEMP_BUFFERS_DRIFT_SYNC_EXIT=$ec"; if grep -Eqi 'temp_buffers|foreign server.*(drift|mismatch|option)' <<<"$out"; then echo 'NEON_FOREIGN_SERVER_OPTIONS_TEMP_BUFFERS_DRIFT_FAIL_CLOSED=true'; exit 0; fi; exit 1; fi
da=$(opt "$D"); row=$(psql "$D" -At -F '|' -c 'select id,sku,quantity from products where id=2'); dp2=$(psql "$DA" -Atc 'select temp_buffers from policy_remote'); printf 'AFTER destination=%s row=%s destination_temp_buffers=%s\nNEON_FOREIGN_SERVER_OPTIONS_TEMP_BUFFERS_DRIFT_SYNC_EXIT=%s\n' "$da" "$row" "$dp2" "$ec"
if [[ "$da" == '-c temp_buffers=16MB' && "$row" == '2|SOURCE-SKU-2|11' && "$dp2" == '16MB' ]]; then echo 'NEON_FOREIGN_SERVER_OPTIONS_TEMP_BUFFERS_DRIFT_DETECTED=true'; exit 0; fi
if [[ "$da" == '-c temp_buffers=8MB' && "$row" == '2|SOURCE-SKU-2|11' && "$dp2" == '8MB' ]]; then echo 'NEON_FOREIGN_SERVER_OPTIONS_TEMP_BUFFERS_DRIFT_DETECTED=false'; echo 'NEON_FOREIGN_SERVER_OPTIONS_TEMP_BUFFERS_POLICY_DIVERGENCE=true'; exit 1; fi
exit 2
