...
create extension postgres_fdw; create table products(id bigint primary key,sku text not null,quantity int not null); create table remote_products(id bigint primary key,sku text not null,quantity int not null); insert into remote_products values(1,'REMOTE-SKU-1',7); create view remote_policy as select current_setting('statement_timeout')::text as statement_timeout; grant usage on schema public to cycle_app; grant select on remote_products,remote_policy to cycle_app;
select format('create server retail_loopback foreign data wrapper postgres_fdw options(host ''127.0.0.1'',port ''5432'',dbname %L,options %L)',:'db','-c statement_timeout=' || :'mode') \gexec
...
sb=$(opt "$S"); db=$(opt "$D"); sr=$(psql "$SA" -At -F '|' -c 'select id,sku,quantity from products_remote'); dr=$(psql "$DA" -At -F '|' -c 'select id,sku,quantity from products_remote'); se=$(psql "$SA" -Atc 'select statement_timeout from policy_remote'); de=$(psql "$DA" -Atc 'select statement_timeout from policy_remote'); printf 'BEFORE source=%s destination=%s source_app=%s destination_app=%s source_statement_timeout=%s destination_statement_timeout=%s\n' "$sb" "$db" "$sr" "$dr" "$se" "$de"
[[ "$sb" == '-c statement_timeout=0' && "$db" == '-c statement_timeout=5s' && "$sr" == '1|REMOTE-SKU-1|7' && "$dr" == '1|REMOTE-SKU-1|7' && "$se" == '0' && "$de" == '5s' ]] || { echo 'Fixture invalid'; exit 2; }
...
da=$(opt "$D"); row=$(psql "$D" -At -F '|' -c 'select id,sku,quantity from products where id=2'); de2=$(psql "$DA" -Atc 'select statement_timeout from policy_remote'); printf 'AFTER destination=%s row=%s destination_statement_timeout=%s\nNEON_FOREIGN_SERVER_OPTIONS_STATEMENT_TIMEOUT_DRIFT_SYNC_EXIT=%s\n' "$da" "$row" "$de2" "$ec"
if [[ "$da" == '-c statement_timeout=0' && "$row" == '2|SOURCE-SKU-2|11' && "$de2" == '0' ]]; then echo 'NEON_FOREIGN_SERVER_OPTIONS_STATEMENT_TIMEOUT_DRIFT_DETECTED=true'; exit 0; fi
if [[ "$da" == '-c statement_timeout=5s' && "$row" == '2|SOURCE-SKU-2|11' && "$de2" == '5s' ]]; then echo 'NEON_FOREIGN_SERVER_OPTIONS_STATEMENT_TIMEOUT_DRIFT_DETECTED=false'; echo 'NEON_FOREIGN_SERVER_OPTIONS_STATEMENT_TIMEOUT_POLICY_DIVERGENCE=true'; exit 1; fi
exit 2