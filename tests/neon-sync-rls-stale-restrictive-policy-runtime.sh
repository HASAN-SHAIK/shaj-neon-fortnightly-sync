...
create policy products_tenant_isolation on public.products as permissive for all to cycle_app
  using (tenant_id = current_setting('app.tenant_id')::integer)
  with check (tenant_id = current_setting('app.tenant_id')::integer);
create policy legacy_store_lock on public.products as restrictive for all to cycle_app
  using (false)
  with check (false);
grant select, insert, update, delete on public.products to cycle_app;
SQL
...
read_legacy_policy_count() {
  ...
  psql "$url" -v ON_ERROR_STOP=1 -Atc "
    ...
    join pg_class c on c.oid=p.polrelid
    join pg_namespace n on n.oid=c.relnamespace
    where n.nspname='public' and c.relname='products' and p.polname='legacy_store_lock';"
}
...
set app.tenant_id='1';
insert into public.products values (901,1,'SOURCE-SAME-TENANT-ALLOWED') returning id,tenant_id,sku;
SQL
)"
source_before_exit=$?
set -e
psql "$SOURCE_POSTGRES_URL" -v ON_ERROR_STOP=1 -c "delete from public.products where id=901;" >/dev/null

set +e
destination_before_output="$(psql "$DESTINATION_APP_URL" -v ON_ERROR_STOP=1 -At -F '|' <<'SQL' 2>&1
set app.tenant_id='1';
insert into public.products values (901,1,'DEST-SAME-TENANT-BEFORE') returning id,tenant_id,sku;
SQL
)"
...
set app.tenant_id='1';
insert into public.products values (902,1,'DEST-SAME-TENANT-AFTER') returning id,tenant_id,sku;
SQL
)"
destination_after_exit=$?
set -e

printf 'NEON_RLS_STALE_RESTRICTIVE_SYNC_EXIT=%s\n' "$sync_exit"
printf 'NEON_RLS_STALE_RESTRICTIVE_SOURCE_POLICY_COUNT_BEFORE=%s\n' "$source_policy_count_before"
printf 'NEON_RLS_STALE_RESTRICTIVE_DESTINATION_POLICY_COUNT_BEFORE=%s\n' "$destination_policy_count_before"
printf 'NEON_RLS_STALE_RESTRICTIVE_DESTINATION_POLICY_COUNT_AFTER=%s\n' "$destination_policy_count_after"
printf 'NEON_RLS_STALE_RESTRICTIVE_DESTINATION_LEGACY_BEFORE=%s\n' "$destination_legacy_before"
printf 'NEON_RLS_STALE_RESTRICTIVE_DESTINATION_LEGACY_AFTER=%s\n' "$destination_legacy_after"
printf 'NEON_RLS_STALE_RESTRICTIVE_SOURCE_WRITE_EXIT=%s\n' "$source_before_exit"
printf 'NEON_RLS_STALE_RESTRICTIVE_SOURCE_WRITE_OUTPUT=%s\n' "$source_before_output"
printf 'NEON_RLS_STALE_RESTRICTIVE_DESTINATION_BEFORE_EXIT=%s\n' "$destination_before_exit"
printf 'NEON_RLS_STALE_RESTRICTIVE_DESTINATION_BEFORE_OUTPUT=%s\n' "$destination_before_output"
printf 'NEON_RLS_STALE_RESTRICTIVE_DESTINATION_AFTER_EXIT=%s\n' "$destination_after_exit"
printf 'NEON_RLS_STALE_RESTRICTIVE_DESTINATION_AFTER_OUTPUT=%s\n' "$destination_after_output"
printf 'NEON_RLS_STALE_RESTRICTIVE_APPENDED_SOURCE_ROW=%s\n' "$destination_source_row_2"

test "$source_policy_count_before" = '1'
test "$destination_policy_count_before" = '2'
test "$destination_legacy_before" = '1'
test "$source_before_exit" -eq 0
grep -Fq '901|1|SOURCE-SAME-TENANT-ALLOWED' <<<"$source_before_output"
test "$destination_before_exit" -ne 0
grep -Eiq 'row-level security|policy' <<<"$destination_before_output"

if [[ "$sync_exit" -ne 0 ]]; then
  if grep -Eiq 'row.level security|policy|legacy_store_lock|products' <<<"$runtime_output"; then
    echo 'NEON_RLS_STALE_RESTRICTIVE_POLICY_SAFE_FAIL_CLOSED=true'
    exit 0
  fi
  echo 'Production sync failed, but not for an explicit RLS/policy incompatibility reason.' >&2
  exit 1
fi

test "$destination_source_row_2" = '2|1|SOURCE-T1-SKU-2'

if [[ "$destination_legacy_after" = '0' && "$destination_policy_count_after" = '1' && "$destination_after_exit" -eq 0 ]] && \
   grep -Fq '902|1|DEST-SAME-TENANT-AFTER' <<<"$destination_after_output"; then
  echo 'NEON_RLS_STALE_RESTRICTIVE_POLICY_DRIFT_DETECTED=true'
  exit 0
fi

echo 'NEON_RLS_STALE_RESTRICTIVE_POLICY_DRIFT_DETECTED=false'
echo 'Destination retained or otherwise failed to neutralize a stale restrictive RLS policy while production sync reported success.' >&2
exit 1
